...
J = 1.0 / m * sum(sum(-Y .* log(a3) - (1 - Y) .* log(1 - a3))) + lambda / 2 / m * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
...
for t = 1:m
    ...
    delta3_t = a3_t - Y(t,:)';
    delta2_t = Theta2(:,2:end)' * delta3_t .* sigmoidGradient(z2_t);
